...
def _resolve_settings_provider(
    ...
    for extra_name, provider_config in _dynamic_provider_items(config):
        if provider_name == extra_name or normalized == extra_name.replace("-", "_"):
            return create_dynamic_spec(extra_name, thinking_style=getattr(provider_config, 'thinking_style', None)), extra_name, provider_config
    return None
    ...
    for provider_key, provider_config in _dynamic_provider_items(config):
        providers.append(
            _provider_settings_row(
                provider_key,
                create_dynamic_spec(provider_key, thinking_style=getattr(provider_config, 'thinking_style', None)),
                provider_config,
            )
        )
